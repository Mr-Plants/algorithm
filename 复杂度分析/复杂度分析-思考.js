...
首先找出这段代码中和n有关系的代码是第5行，因为代码的执行时间T(n)与代码的执行次数N成正比，
所以如果能知道第5行代码执行了多少次，就知道了执行时间与数量规模n的关系。
...
递归函数一般是2^n
循环嵌套一般是n^2或者m*n
常见的复杂度为O(1),O(n),O(n^2),O(m*n),O(log n),O(2^n),O(n*log n)。忽略常系数等影响，最终应该转化为其中一个
 */
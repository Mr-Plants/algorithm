...
// TODO(pwong): Remove once TypedArray.prototype.join() is ported to Torque.
  function SparseJoinWithSeparatorJS(
    ...
    return
  %
    SparseJoinWithSeparator(elements, length, separator);
  }
...
// Optimized for sparse arrays if separator is ''.
  function SparseJoin(array, keys, use_locale, locales, options) {
    ...
    return
  %
    StringBuilderConcat(elements, keys_length, '');
  }
...
// TODO(pwong): Remove once TypedArray.prototype.join() is ported to Torque.
  function UseSparseVariant(array, length, is_array, touched) {
    ...
    var estimated_elements =
  %
    EstimateNumberOfElements(array);
...
// TODO(pwong): Remove once TypedArray.prototype.join() is ported to Torque.
  function DoJoin(
    ...
    if (separator === '') {
      return
    %
      StringBuilderConcat(elements, length, '');
    } else {
      return
    %
...
// TODO(pwong): Remove once TypedArray.prototype.join() is ported to Torque.
  function InnerArraySort(array, length, comparefn) {
    ...
    var num_non_undefined =
  %
    PrepareElementsForSort(array, length);
